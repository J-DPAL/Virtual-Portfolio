...
public class DataSeeder implements CommandLineRunner {
  ...
  public void run(String... args) throws Exception {
    try {
      ...
      String checkProjectSql =
          "select 1 from public.projects where lower(title_en)=lower(:title) limit 1";
      Integer p = null;
      try {
        p =
            jdbc.queryForObject(
                checkProjectSql, new MapSqlParameterSource("title", "SmartCD.ai"), Integer.class);
      } catch (Exception ex) {
        // queryForObject may throw when result is empty; swallow here and treat as missing
      }
      if (p == null) {
        String insertProject =
            "insert into public.projects (title_en,title_fr,title_es,description_en,description_fr,description_es,technologies,project_url,github_url,image_url,status) values (:t,:t,:t,:de,:df,:ds,:tech,:url,:gh,:img,:status)";
      ...
      String checkExpSql =
          "select 1 from public.work_experience where lower(company_name_en)=lower(:company) and lower(position_en)=lower(:position) limit 1";
      Integer e = null;
      try {
        e =
            jdbc.queryForObject(
                checkExpSql,
                new MapSqlParameterSource()
                    .addValue("company", "Product Manager Accelerator")
                    .addValue("position", "Full-Stack AI Engineering Intern"),
                Integer.class);
      } catch (Exception ex) {
        // treat as missing
      }
      ...
      log.warn(
          "DataSeeder skipped: database unavailable or error during seeding - {}", ex.getMessage());
    }
  }
}

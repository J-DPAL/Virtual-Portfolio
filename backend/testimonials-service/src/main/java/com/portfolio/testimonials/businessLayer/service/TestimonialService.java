...
public class TestimonialService {
    ...
    public List<TestimonialDTO> getApprovedTestimonials() {
        return testimonialRepository.findByStatusOrderByCreatedAtDesc(TestimonialStatus.APPROVED).stream()
                .map(testimonialMapper::toDTO)
                .collect(Collectors.toList());
    }

    public List<TestimonialDTO> getPendingTestimonials() {
        return testimonialRepository.findByStatusOrderByCreatedAtDesc(TestimonialStatus.PENDING).stream()
                .map(testimonialMapper::toDTO)
                .collect(Collectors.toList());
    }
